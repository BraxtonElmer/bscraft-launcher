...
export function createMocks() {
  ...
  const handlers = {
    ...
    // Live server status: ?srv=down|empty|busy|hidden (default: 3 players, including the mock user).
    // Players come and go in the preview: set window.__players = [...names] from the console,
    // or window.__freezeStatus = true to stop further answers (e.g. while poking at window.__party).
    server_status: async () => {
      if (window.__freezeStatus) return new Promise(() => {})
      await sleep(350)
      const srv = params.get('srv')
      if (srv === 'down') return { online: false, players_online: 0, players_max: 0, players: [], latency_ms: 0, version: '' }
      const names = window.__players ?? (srv === 'empty' ? [] : srv === 'busy'
        ? ['Akariyu', 'Blaze_Runner', 'Crafty', 'Dewdrop', 'Ember', 'Frostbyte', 'Gravel', 'Hazel', 'Ivy_', 'Jade', 'Kiln', 'Raxtray']
        : srv === 'hidden' ? ['Akariyu'] : ['Akariyu', 'Raxtray', 'Zukashi'])
      const online = srv === 'busy' ? 15 : srv === 'hidden' ? 3 : names.length
