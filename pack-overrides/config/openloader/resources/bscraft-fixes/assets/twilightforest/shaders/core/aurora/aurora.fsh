...
vec3 grad(float hash) {
    ...
    // (Zeroes component int(hash / 16.0); written as a mask because a dynamically
    // indexed vector write crashes Intel's driver when linking, see TF issue #2559)
    vec3 cuboct = cube * vec3(notEqual(ivec3(int(hash / 16.0)), ivec3(0, 1, 2)));
